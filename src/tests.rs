...
use crate::{
	...
	assert_last_event, assert_tail_eq,
	mock::{
	...
	pallet::CapacityStatus,
	set::OrderedSet,
	AtStake, BalanceOf, Bond, BottomDelegations, CandidateInfo, CandidateMetadata, CandidatePool,
	CandidateState, CollatorCandidate, CollatorStatus, Config, DelegationChange, DelegationRequest,
...
fn set_blocks_per_round_fails_if_below_total_selected() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(ParachainStaking::set_blocks_per_round(Origin::root(), 20u32));
...
fn set_inflation_event_emits_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(ParachainStaking::set_inflation(Origin::root(), Range { min, ideal, max }));
...
fn set_parachain_bond_account_event_emits_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(ParachainStaking::set_parachain_bond_account(Origin::root(), 11));
...
fn delegator_bond_less_updates_delegator_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_ok!(ParachainStaking::schedule_delegator_bond_less(Origin::signed(2), 1, 5));
			let state =
				ParachainStaking::delegator_state(&2).expect("just request bonded less so exists");
			assert_eq!(
...
fn can_delegator_bond_less_if_leaving() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_ok!(ParachainStaking::schedule_leave_delegators(Origin::signed(2)));
			assert_ok!(ParachainStaking::schedule_delegator_bond_less(Origin::signed(2), 1, 1));
		});
}
...
fn delegator_bond_more_after_revoke_delegation_does_not_effect_exit() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_ok!(ParachainStaking::schedule_revoke_delegation(Origin::signed(2), 1));
			assert_ok!(ParachainStaking::delegator_bond_more(Origin::signed(2), 3, 10));
			roll_to(10);
			assert_ok!(ParachainStaking::execute_delegation_request(Origin::signed(2), 2, 1));
			assert!(ParachainStaking::is_delegator(&2));
...
fn delegator_bond_less_after_revoke_delegation_does_not_effect_exit() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(ParachainStaking::schedule_delegator_bond_less(Origin::signed(2), 3, 2));
			roll_to(10);
			assert_ok!(ParachainStaking::execute_delegation_request(Origin::signed(2), 2, 1));
			assert_ok!(ParachainStaking::execute_delegation_request(Origin::signed(2), 2, 3));
...
fn execute_delegator_bond_less_updates_just_bottom_delegations() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(ParachainStaking::schedule_delegator_bond_less(Origin::signed(2), 1, 2));
			roll_to(10);
			assert_ok!(ParachainStaking::execute_delegation_request(Origin::signed(2), 2, 1));
...
fn execute_delegator_bond_less_does_not_delete_bottom_delegations() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let pre_call_bottom_delegations =
				ParachainStaking::bottom_delegations(&1).expect("delegated by all so exists");
			assert_ok!(ParachainStaking::schedule_delegator_bond_less(Origin::signed(6), 1, 4));
			roll_to(10);
			assert_ok!(ParachainStaking::execute_delegation_request(Origin::signed(6), 6, 1));
			let post_call_candidate_info =
				ParachainStaking::candidate_info(&1).expect("delegated by all so exists");
			let post_call_top_delegations =
				ParachainStaking::top_delegations(&1).expect("delegated by all so exists");
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_last_event!(MetaEvent::ParachainStaking(Event::CancelledDelegationRequest {
				delegator: 2,
				cancelled_request: DelegationRequest {
					collator: 1,
					amount: 10,
					when_executable: 3,
...
fn delegator_schedule_revocation_total() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_ok!(ParachainStaking::schedule_revoke_delegation(Origin::signed(2), 1));
			assert_eq!(
				ParachainStaking::delegator_state(2).expect("exists").requests.less_total,
				10
			);
			roll_to(10);
			assert_ok!(ParachainStaking::execute_delegation_request(Origin::signed(2), 2, 1));
			...
			assert_ok!(ParachainStaking::execute_delegation_request(Origin::signed(2), 2, 4));
			assert_eq!(
				ParachainStaking::delegator_state(2).expect("exists").requests.less_total,
				0
			);
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let mut new3 = vec![
				...
				Event::Rewarded { account: 1, rewards: 20 },
				Event::Rewarded { account: 6, rewards: 4 },
...
fn execute_leave_candidate_removes_delegations() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(ParachainStaking::delegator_state(3).unwrap().requests.revocations_count, 0);
			...
			assert_eq!(ParachainStaking::delegator_state(3).unwrap().requests.revocations_count, 1);
...
fn candidate_pool_updates_when_total_counted_changes() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(ParachainStaking::delegator_bond_more(Origin::signed(3), 1, 8));
			// 3: 11 -> 19 => 3 is in top, bumps out 7
			// 16 + 17 + 18 + 19 + 20 = 90 (top 4 + self bond)
			is_candidate_pool_bond(1, 90);
			assert_ok!(ParachainStaking::delegator_bond_more(Origin::signed(4), 1, 8));
			// 4: 12 -> 20 => 4 is in top, bumps out 8
			// 17 + 18 + 19 + 20 + 20 = 94 (top 4 + self bond)
			is_candidate_pool_bond(1, 94);
			assert_ok!(ParachainStaking::schedule_delegator_bond_less(Origin::signed(10), 1, 3));
			roll_to(30);
			// 10: 18 -> 15 => 10 bumped to bottom, 8 bumped to top (- 18 + 16 = -2 for count)
			assert_ok!(ParachainStaking::execute_delegation_request(Origin::signed(10), 10, 1));
			// 16 + 17 + 19 + 20 + 20 = 92 (top 4 + self bond)
			is_candidate_pool_bond(1, 92);
			assert_ok!(ParachainStaking::schedule_delegator_bond_less(Origin::signed(9), 1, 4));
			roll_to(40);
			assert_ok!(ParachainStaking::execute_delegation_request(Origin::signed(9), 9, 1));
...
fn only_top_collators_are_counted() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// bump bottom to the top
			assert_ok!(ParachainStaking::delegator_bond_more(Origin::signed(5), 1, 8));
			assert_event_emitted!(Event::DelegationIncreased {
				delegator: 5,
				candidate: 1,
				amount: 8,
			...
			// bump bottom to the top
			assert_ok!(ParachainStaking::delegator_bond_more(Origin::signed(6), 1, 8));
			assert_event_emitted!(Event::DelegationIncreased {
				delegator: 6,
				candidate: 1,
				amount: 8,
...
fn delegation_events_convey_correct_position() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(ParachainStaking::delegator_bond_more(Origin::signed(8), 1, 3));
			assert_event_emitted!(Event::DelegationIncreased {
				delegator: 8,
				candidate: 1,
				amount: 3,
			...
			assert_ok!(ParachainStaking::delegator_bond_more(Origin::signed(3), 1, 1));
			assert_event_emitted!(Event::DelegationIncreased {
				delegator: 3,
				candidate: 1,
				amount: 1,
			...
			assert_ok!(ParachainStaking::schedule_delegator_bond_less(Origin::signed(6), 1, 2));
			...
			assert_ok!(ParachainStaking::schedule_delegator_bond_less(Origin::signed(6), 1, 1));
			assert_event_emitted!(Event::DelegationDecreaseScheduled {
				delegator: 6,
				candidate: 1,
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let corrupted_delegator_state = Delegator {
				id: 2,
				delegations: OrderedSet::from(vec![Bond { owner: 1, amount: 20 }]),
				total: 20,
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			<CandidatePool<Test>>::put(OrderedSet::from(vec![
				...
				Bond { owner: 2, amount: 16 },
				Bond { owner: 3, amount: 17 },
				Bond { owner: 4, amount: 18 },
				Bond { owner: 5, amount: 19 },
			]));
		...
		.execute_with(|| {
			...
			for i in 1..3 {
				let old_candidate_state = CollatorCandidate {
					...
					top_delegations: vec![
						Bond { owner: 3, amount: 19 },
						Bond { owner: 4, amount: 18 },
						Bond { owner: 5, amount: 17 },
						Bond { owner: 6, amount: 16 },
					],
					bottom_delegations: vec![
						Bond { owner: 12, amount: 10 },
						Bond { owner: 11, amount: 11 },
						Bond { owner: 10, amount: 12 },
						Bond { owner: 9, amount: 13 },
						Bond { owner: 8, amount: 14 },
						Bond { owner: 7, amount: 15 },
					],
					total_counted: 90,
					total_backing: 165,
					request: None,
			...
			assert_event_emitted!(Event::DelegatorLeft { delegator: 12, unstaked_amount: 10 });
			assert_event_emitted!(Event::DelegatorLeft { delegator: 11, unstaked_amount: 11 });
...
fn split_candidate_state_kicks_extra_bottom_delegations_without_exit() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_event_not_emitted!(Event::DelegatorLeft { delegator: 12, unstaked_amount: 10 });
			assert_event_not_emitted!(Event::DelegatorLeft { delegator: 11, unstaked_amount: 10 });
			// kicked 11 and 12 and revoked them
			assert_eq!(Balances::free_balance(&11), 22);
			assert_eq!(Balances::free_balance(&12), 20);
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			for i in 1..3 {
				...
				assert_eq!(
					top_delegations.delegations,
					vec![Bond { owner: 3, amount: 10 }, Bond { owner: 4, amount: 10 }]
				);
				let bottom_delegations = <BottomDelegations<Test>>::get(&i).unwrap();
				assert_eq!(bottom_delegations.total, 0);
				assert!(bottom_delegations.delegations.is_empty());
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			for i in 1..3 {
				...
				assert_eq!(
					bottom_delegations.delegations,
					vec![Bond { owner: 7, amount: 15 }, Bond { owner: 8, amount: 14 }]
				);
				let candidate_metadata = <CandidateInfo<Test>>::get(&i).unwrap();
				assert_eq!(candidate_metadata.top_capacity, CapacityStatus::Full);
...
fn delegation_kicked_from_bottom_removes_pending_request() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// ensure request DNE
			assert!(ParachainStaking::delegator_state(&2).unwrap().requests().get(&1).is_none());
		});
}
